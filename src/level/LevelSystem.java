    ...
    public void update() {
        for (int i = 0; i < lc.size; i++) {
            for (int j = 0; j < lc.size; j++) {
                ...
                Graphics3D.fillRect(new Vec3(i, j, t.height), 1, 1, 0, 0, WHITE);
                if (i > 0) {
                    Tile t1 = lc.tileGrid[i - 1][j];
                    if (t1.height != t.height) {
                        Graphics3D.fillRect(new Vec3(i, j, t.height), 1, t1.height - t.height, 90, 90, WHITE);
                    }
                }
                if (j > 0) {
                    Tile t1 = lc.tileGrid[i][j - 1];
                    if (t1.height != t.height) {
                        Graphics3D.fillRect(new Vec3(i + 1, j, t.height), -1, t1.height - t.height, 90, 0, WHITE);
                    }
                }
            }
        }
